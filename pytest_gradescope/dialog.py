...
from pytest_gradescope.edit_dist import edit_dist
...
def _extract_groups(dialog_contents: str):
    ...
    group_names = {DEFAULT_GROUP: DEFAULT_GROUP_NAME}
...
def _score_observed_output(expected_output, observed_output):
    group_weights, group_names, group_sequence, dialog_contents = _extract_groups(
        expected_output
    )

    _, obs, exp = edit_dist(observed_output, dialog_contents, GAP=GAP)
    ...
    group_obs[DEFAULT_GROUP] = pad(obs.replace(GAP, ""))
    group_exp[DEFAULT_GROUP] = pad(exp.replace(GAP, ""))
    ...
    for group_id, group_name in group_names.items():
        ...
        group_stats[group_name] = {
            ...
            "score": group_matches.get(group_id, 0)
...
def _score_output(
    expected_io: str, observed_io: str, expected_files: list[tuple[Path, Path]]
):
    ...
    for exp_file, obs_file in expected_files:
        if not obs_file.exists():
            obs_content = (
                f"File not found: {obs_file}. Did you write it?\n" + observed_io
            )
        else:
            obs_content = obs_file.read_text()
...
async def _run_with_timeout(waitable, timeout):
    ...
    t1.add_done_callback(
        lambda t: t2.remove_done_callback(_timeout) and fut.set_result(t.result)
    )
...
async def _run_exec_with_io(
    ...
    def kill(t):
        error.append(
            "The program failed to finish in the expected amount of time; do you have an infinite loop?"
        )
        proc.kill()
    ...
    output.append(
        await _read_stream(proc.stdout, read_timeout, max_output_size - output_size)
    )
    ...
    for i in range(len(inputs)):
        ...
        response = await _read_stream(
            proc.stdout, read_timeout, max_output_size - output_size
        )
        ...
        if output_size > max_output_size:
            error.append(
                f"Program output exceeded limit of {max_output_size} characters"
            )
            proc.kill()
            break
...
def _run_script(
    script_name, *args, inputs: list[str] = None, module="__main__", echo_output=True
):
    if inputs is None:
        inputs = []
    ...
    _globals = {"input": _py_input, "print": _py_print, "sys": sys}
...
def _run_dialog(
    ...
    **kwargs,
    ...
    expected_files = [(to_path(ex), to_path(ob)) for ex, ob in (expected_files or [])]
    ...
    try:
        ...
        output = runner(executable, *args, inputs=inputs, **kwargs)

        # Score results
        group_stats = _score_output(expected_io, output, expected_files)
...
def dialog_exec(
    dialog_file, executable, *args, output_file=None, read_timeout=1, **deprecated
):
    if deprecated:
    ...
    if output_file is not None:
        ...
        group_stats = run_exec(
            executable, *args, expected_stdio=dialog_file, read_timeout=read_timeout
        )
...
def dialog(dialog_file, script, *script_args, output_file=None):
    if output_file is not None:
        group_stats = run_script(
            script, *script_args, expected_files=[(dialog_file, output_file)]
        )
    else:
...
def record_script(dialog_file, script_name, *script_args):
    ...
    with open(dialog_file, "w") as file:
        ...
        _globals = {"input": _input, "print": _print, "sys": sys}
...
def record_exec(dialog_file, executable, *args):
    ...
    args = [executable, *(str(a) for a in args)]
    with open(dialog_file, "w") as file:
        process = sp.Popen(args, stdin=sp.PIPE, stdout=sp.PIPE, stderr=sp.STDOUT)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "args", nargs="*", help="Arguments (if any) to the Python script or executable"
    )
    parser.add_argument(
        "-e",
        "--exec",
